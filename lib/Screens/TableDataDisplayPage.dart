...
class _DataDisplayPageState extends State<DataDisplayPage> {
  ...
  Future<void> _checkBluetoothStatus() async {
    bool isBluetoothOn = await BluetoothEnable.enableBluetooth.then((result) async {
      print("Bluetooth IS On Stage one");
      await _connectToBluetoothPrinter();
      return result == "true";
    });
  ...
  Future<List<Map<String, dynamic>>> fetchTableData(String latestInvoiceId) async {
    try {
      ...
      print("Invoice ID ${latestInvoiceId}");
      print('Error fetching table data: $e');
      throw Exception('Please add the data to the list');
    }
  }
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Column(
              children: [
                  ...
                  children: [
                    TableRow(
                      children: [
                        ...
                        TableCell(child: Center(child: Text('Item Name', style: TextStyle(fontSize: responsiveFontSize)))),
                    ...
                    for (int index = 0; index < tableData.length; index++)
                      TableRow(
                        children: [
                          TableCell(child: Center(child: Text((index + 1).toString(), style: TextStyle(fontSize: responsiveFontSize)))),
                          TableCell(child: Text('${tableData[index]['item_name']}', style: TextStyle(fontSize: responsiveFontSize))),
                          TableCell(child: Center(child: Text('${tableData[index]['item_price']}', style: TextStyle(fontSize: responsiveFontSize)))),
                          TableCell(child: Center(child: Text('${tableData[index]['qty']}', style: TextStyle(fontSize: responsiveFontSize)))),
                          TableCell(
                            child: Center(
                              child: Text('${tableData[index]['item_amt']}', style: TextStyle(fontSize: responsiveFontSize)),
                            ),
                          ),
                        ],
                      ),
                ...
                ElevatedButton(
                  onPressed: _printData,
                  child: Text('Print', style: TextStyle(fontSize: responsiveFontSize)),
                  style: ButtonStyle(
                    backgroundColor: MaterialStateProperty.all<Color>(Colors.orange),
                  ),
                ),
